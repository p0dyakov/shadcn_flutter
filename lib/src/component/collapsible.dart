...
class CollapsibleTrigger extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      Expanded(child: child.text2().textW6()),
      Gap(compTheme?.iconGap ?? 16 * scaling),
      GhostButton(
        onPressed: state.handleTap,
        child: Icon(
          state.isExpanded
              ? compTheme?.iconExpanded ?? BootstrapIcons.arrowDown
              : compTheme?.iconCollapsed ?? BootstrapIcons.arrowUp,
        ).icon1(),
      ),
    ]).withPadding(horizontal: horizontalPadding ?? compTheme?.padding ?? 16 * scaling);
  }
}
