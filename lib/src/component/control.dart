...
class ControlledComponentAdapter<T> extends StatefulWidget with ControlledComponent<T> {
  ...
  const ControlledComponentAdapter({
    ...
    this.controller,
    this.enabled = true,
  }) : assert(controller != null || initialValue is T, 'Either controller or initialValue must be provided');

  @override
  _ControlledComponentAdapterState<T> createState() => _ControlledComponentAdapterState<T>();
}

class _ControlledComponentAdapterState<T> extends State<ControlledComponentAdapter<T>> {
  late T _value;
