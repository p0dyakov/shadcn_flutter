...
class ObjectFormFieldState<T> extends State<ObjectFormField<T>> with FormValueSupplier<T, ObjectFormField<T>> {
  ...
  Widget build(BuildContext context) {
    return OutlineButton(
      trailing: widget.trailing?.iconMutedForeground().icon2(),
      leading: widget.leading?.iconMutedForeground().icon2(),
      size: widget.size,
      density: widget.density,
      shape: widget.shape,
      onPressed: widget.onChanged == null ? null : prompt,
      enabled: enabled,
      child: this.value == null ? widget.placeholder.textMuted() : widget.builder(context, this.value as T),
    );
  }
}
